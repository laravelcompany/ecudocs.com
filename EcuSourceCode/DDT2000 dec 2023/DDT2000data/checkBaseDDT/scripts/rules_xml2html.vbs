' ------------------------------------------------------------
' Create an html rules file, by applying XSLT transformation
...
Const TOOL_NAME = "HTML rules file generator"
...
xslFilename    = SOURCE_PATH  & "rules_displayCheckBaseDDT.xsl"
  'replace ".report.xml" from input parameter 1 (objArgs(0)) given by builder by ".rules.html"
outputFilename = Replace(objArgs(0), ".report.xml", ".rules.html")
...
'*************************************
If verbose Then
	msgbox "+ xmlFilename: " & vbnewline & xmlFilename & vbnewline & "+ xslFilename: " & vbnewline & xslFilename, vbInformation, "Received parameters"
End If
...
Else
  If (Not xmlDoc.load(xmlFilename)) Then
    msgBox "Error on loading XML report file:" & vbnewline & xslFileName, vbOKOnly, TOOL_NAME
    wscript.quit
  End If
End If
...
' ********************************************
' Configure converter xml report -> html rules
' ********************************************
...
'*********************************************
' Convert XML report file into html rules file
'*********************************************
If Not (xslProc.Transform()) Then
  msgBox "Unable to generate html rules file :" & vbnewline & outputFilename, vbOKOnly, TOOL_NAME
Else
