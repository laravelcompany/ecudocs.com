...
<xsl:stylesheet version="1.0" exclude-result-prefixes="msxsl ddt ds"
  ...
  <xsl:template name="checkDataItemNumeric">
    <xsl:param name="type" />
    ...
    <xsl:choose>
      <xsl:when test="//ddt:Target/ddt:Datas/ddt:Data[@Name=$dataName]/ddt:Bytes">
      ...
      <xsl:when test="ddt:Sent/ddt:DataItem[@Name = $dataName]">
        <xsl:apply-templates select="ddt:Sent/ddt:DataItem[@Name = $dataName]" mode="Numeric">
      ...
      <xsl:when test="ddt:Received/ddt:DataItem[@Name = $dataName]">
        <xsl:apply-templates select="ddt:Received/ddt:DataItem[@Name = $dataName]" mode="Numeric">
            <xsl:with-param name="mType"><xsl:value-of select="$type" /></xsl:with-param>
            <xsl:with-param name="mChapter"><xsl:value-of select="$chapter" /></xsl:with-param>
            <xsl:with-param name="rName"><xsl:value-of select="$reqName" /></xsl:with-param>
            <xsl:with-param name="sByte"><xsl:value-of select="$startByte" /></xsl:with-param>
      ...
      <xsl:otherwise>
          <xsl:call-template name="error">
            ...
            <xsl:with-param name="action">
            <!--Data Definition : <b><xsl:value-of select="$dataName" /></b><br/>
            <xsl:value-of select="$msgMissing" />-->
                  <xsl:call-template name="getLocalizedText">
              <xsl:with-param name="lang"><xsl:value-of select="$language"></xsl:value-of></xsl:with-param>
  ...
  <xsl:template match="ddt:DataItem" mode="Numeric">
  ...
  <!--
    <xsl:choose>
      <xsl:when test="key('allDatas',$nom)">
        <xsl:if test = "//ddt:Target/ddt:Datas/ddt:Data[@Name=$nom]/ddt:Bits[@count != $nbBits]">
          <xsl:call-template name="error">
            <xsl:with-param name="description"><xsl:value-of select="$rName" /> Data : <xsl:value-of select="$nom" /></xsl:with-param>
            <xsl:with-param name="info">bad value Numbers bits :  <xsl:value-of select="//ddt:Target/ddt:Datas/ddt:Data[@Name=$nom]/ddt:Bits/@count" /></xsl:with-param>
            <xsl:with-param name="action">Normal value numbers bits : <xsl:value-of select="$nbBits" /></xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
    </xsl:choose>
  -->
          ...
          <xsl:when test = "//ddt:Target/ddt:Datas/ddt:Data[@Name=$nom]/ddt:Bits[@count != $nbBits]">
            <xsl:call-template name="error">
              ...
              <xsl:with-param name="info">
                  ...
                  <xsl:with-param name="defaultText">*non trouvé*</xsl:with-param>
                </xsl:call-template> = <b><xsl:value-of select="//ddt:Target/ddt:Datas/ddt:Data[@Name=$nom]/ddt:Bits/@count" /></b><br/>
                <b>
                      <xsl:call-template name="getLocalizedText">
                  <xsl:with-param name="lang"><xsl:value-of select="$language"></xsl:value-of></xsl:with-param>
                  <xsl:with-param name="aNode" select="document('Translate.xml')/translation/infoInvalidValue" />
              ...
              <xsl:with-param name="action">
                  ...
                  <xsl:with-param name="aNode" select="document('Translate.xml')/translation/infoNbBit1" />
                  <xsl:with-param name="defaultText">*non trouvé*</xsl:with-param>
                </xsl:call-template> = <b><xsl:value-of select="$nbBits" />
              </b></xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
    ...
    <xsl:choose>
      ...
      <xsl:when test = "//ddt:Target/ddt:Datas/ddt:Data[@Name=$nom]/ddt:Bits[@signed != $signed]">
      <!-- définition contrôle signed=0(no signed) et data dans la base signed=1 Check = KO
           Valeur attendue = no signed-->
        <xsl:call-template name="error">
          <xsl:with-param name="type"><xsl:value-of select="$mType" /></xsl:with-param>
          <xsl:with-param name="chapter"><xsl:value-of select="$mChapter" /></xsl:with-param>
          <xsl:with-param name="description">
      ...
      <!-- définition contrôle signed=1 et data dans la base no signed (pas de paramètre @signed) Check = KO
           Valeur attendue = signed-->
        <xsl:call-template name="error">
          <xsl:with-param name="type"><xsl:value-of select="$mType" /></xsl:with-param>
          <xsl:with-param name="chapter"><xsl:value-of select="$mChapter" /></xsl:with-param>
          <xsl:with-param name="description">
