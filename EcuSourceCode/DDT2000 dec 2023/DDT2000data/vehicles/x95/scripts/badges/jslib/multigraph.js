...
function MWJ_graph(oW,oH,oType,oLegend,oRotate,oColor) {
	...
	this.setYAxis = function (oStep) {
		...
		if( typeof(oStep) != 'number' || this.type == 2 || oStep <= 0 ) {
			this.error = 'setYAxis was incorrectly used'; if( !MWJ_graphError ) { alert( 'setYAxis instruction failed\n'+((this.type==2)?'this method does not apply to relative percentage bar charts':'expected a positive number')+MWJ_errorMsg ); } MWJ_graphError = true; return; }
		this.YAxis = oStep;
	}
	this.buildGraph = function () {
		...
		document.write('<table class="'+ this.className +'" cellpadding="0" cellspacing="7" border="0">');
		var temp = 1 + ( leftTitle ? 1 : 0 ) + ( this.legend ? 1 : 0 );
		if( this.graphTitle ) { document.write('<tr><th'+((temp>1)?(' colspan="'+temp+'"'):'')+(document.layers&&!document.getElementById?'><font size="+1"':' style="font-size:larger;"')+'>'+this.graphTitle+(document.layers&&!document.getElementById?'<\/font>':'')+'<\/th><\/tr>\n'); }
		...
		document.write('<table cellpadding="0" cellspacing="0" border="0"><tr><td rowspan="2" valign="top">'+(document.layers&&!document.getElementById?('<ilayer height="'+this.height+'">'):('<div style="position:relative;left:0px;top:0px;height:'+this.height+'px;">')));
		//start left scale
		...
		document.write((document.layers&&!document.getElementById?'<\/ilayer>':'<\/div>')+'<\/td><td height="'+this.height+'" width="'+this.width+'"'+(isIE5Mac?(' style="position:relative;left:20px;top:0px;" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'"><div style="position:absolute;">'):(document.layers&&!document.getElementById?('><ilayer left="0" top="0" width="'+this.width+'" height="'+this.height+'" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'">'):('><div style="position:relative;top:0px;left:0px;width:'+this.width+'px;height:'+this.height+'px;background-color:'+(this.bgColor?this.bgColor:'#bfbfbf')+';">'))));
		if( document.layers && !document.getElementById && !navigator.mimeTypes['*'] ) { document.write('<layer left="0" top="0"><table cellpadding="0" cellspacing="0" border="0"><tr><td height="'+this.height+'" width="'+this.width+'" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'">&nbsp;</td></tr></table></layer>'); }
		//start grid
		...
		//end bars
		document.write((document.layers&&!document.getElementById?'<\/ilayer>':'<\/div>')+'<\/td><\/tr><tr><td valign="top"'+(isIE5Mac?' height="20"':'')+'>'+(document.layers&&!document.getElementById?('<ilayer width="'+this.width+'" height="20">'):('<div style="position:relative;left:0px;top:0px;height:20px;width:'+this.width+'px;">')));
		//start bottom scale
		if( this.rotate ) {
		...
		document.write((document.layers&&!document.getElementById?'<\/ilayer>':'<\/div>')+'<\/td><\/tr><\/table><\/td>');
	...
	this.buildGraphString = function () {
		var s = "";
		if( !this.error && !this.data.length ) {
			this.error = 'no data was provided'; if( !MWJ_graphError ) { alert( 'Graph drawing failed\nno data sets were provided so there is nothing to plot'+MWJ_errorMsg ); } MWJ_graphError = true; }
		if( !this.error && this.data.length < 2 && ( this.type == 1 || this.type == 2 ) ) {
			this.error = 'a minimum of two data sets were expected'; if( !MWJ_graphError ) { alert( 'Graph drawing failed\na minimum of two data sets are required for the chosen graph type'+MWJ_errorMsg ); } MWJ_graphError = true; }
		if( this.error ) { s += ('<div><font color="#ff0000">Graph drawing failed because '+this.error+'.<\/font><\/div>'); return(s); }
		var leftTitle = this.rotate ? this.XTitle : this.YTitle;
		var botTitle = this.rotate ? this.YTitle : this.XTitle;
		s += ('<table class="'+ this.className +'" cellpadding="0" cellspacing="7" border="0">');
		var temp = 1 + ( leftTitle ? 1 : 0 ) + ( this.legend ? 1 : 0 );
		if( this.graphTitle ) { s += ('<tr><th'+((temp>1)?(' colspan="'+temp+'"'):'')+(document.layers&&!document.getElementById?'><font size="+1"':' style="font-size:larger;"')+'>'+this.graphTitle+(document.layers&&!document.getElementById?'<\/font>':'')+'<\/th><\/tr>\n'); }
		s += ('<tr>'); var temp = 1 + ( botTitle ? 1 : 0 );
		if( leftTitle && typeof(leftTitle) == 'string' ) {
			s += ('<th'+((temp>1)?(' rowspan="'+temp+'"'):'')+'>');
			for( var x = 0, y = ''; leftTitle.charAt(x); x++ ) { y += leftTitle.charAt(x) + ( leftTitle.charAt(x+1) ? '<br>' : '' ); }
			s += (y+'<\/th>\n');
		}
		s += ('<td>');
		...
		s += ('<table cellpadding="0" cellspacing="0" border="0"><tr><td rowspan="2" valign="top">'+(document.layers&&!document.getElementById?('<ilayer height="'+this.height+'">'):('<div style="position:relative;left:0px;top:0px;height:'+this.height+'px;">')));
		//start left scale
		if( this.rotate ) { for( var x = 0; this.XAxis && x < this.XAxis.length; x++ ) { if( this.XAxis[x] ) {  s += (MWJ_giveWords(0,( this.height * ( ( ( 2 * x ) + 1 ) / ( 2 * this.XAxis.length ) ) ) - 7,this.XAxis[x])); } } } else {
			for( var x = 0; x < sideTicks.length; x++ ) { var y = (this.height*((maxH-sideTicks[x])/grRange))-7;
				if( sideTicks[x] == minH ) { y -= 5; } if( sideTicks[x] == maxH ) { y += 4; }  s += (MWJ_giveWords(0,y,sideTicks[x]+((this.type==2)?'%':''))); }
			var y = (this.height*(maxH/grRange))-7;
			if( !minH ) { y -= 5; } if( !maxH ) { y += 4; }  s += (MWJ_giveWords(0,y,0+((this.type==2)?'%':'')));
		}
		//end left scale - as for the IE5 Mac fix, don't ask and don't argue, it just works OK!
		var isIE5Mac = document.getElementById && window.ActiveXObject && !navigator.__ice_version && navigator.userAgent.indexOf('Mac') + 1 && window.ScriptEngine && ScriptEngine() == 'JScript';
		s += ((document.layers&&!document.getElementById?'<\/ilayer>':'<\/div>')+'<\/td><td height="'+this.height+'" width="'+this.width+'"'+(isIE5Mac?(' style="position:relative;left:20px;top:0px;" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'"><div style="position:absolute;">'):(document.layers&&!document.getElementById?('><ilayer left="0" top="0" width="'+this.width+'" height="'+this.height+'" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'">'):('><div style="position:relative;top:0px;left:0px;width:'+this.width+'px;height:'+this.height+'px;background-color:'+(this.bgColor?this.bgColor:'#bfbfbf')+';">'))));
		if( document.layers && !document.getElementById && !navigator.mimeTypes['*'] ) { s += ('<layer left="0" top="0"><table cellpadding="0" cellspacing="0" border="0"><tr><td height="'+this.height+'" width="'+this.width+'" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'">&nbsp;</td></tr></table></layer>'); }
		//start grid
		if( this.type == 3 ) { for( var x = 0; x < botTicks.length; x++ ) { s += (MWJ_giveBar(10,(this.width*((botTicks[x]-minW)/grWange))-1,(this.height*(maxH/grRange))-5,false)); }
		} else { for( var x = 0; x < botTicks.length; x++ ) { s += (MWJ_giveBar(grSz,(this.rotate?0:(botTicks[x]-1)),(this.rotate?(botTicks[x]-1):0),this.rotate)); } }
		for( var x = 0; x < sideTicks.length; x++ ) { var y = (this.rotate?(this.width*((sideTicks[x]-minH)/grRange)):(this.height*((maxH-sideTicks[x])/grRange)))-1; s += (MWJ_giveBar(10,(this.rotate?y:((this.type==3)?(((-1*minW*this.width)/grWange)-5):-5)),(this.rotate?(this.height-5):y),!this.rotate)); }
		var y = (this.rotate?(this.width*((0-minH)/grRange)):(this.height*(maxH/grRange)))-1; s += (MWJ_giveBar((this.rotate?(this.height+5):(this.width+((this.type==3)?0:5))),(this.rotate?y:((this.type==3)?0:-5)),(this.rotate?0:y),!this.rotate));
		if( this.type == 3 ) { s += (MWJ_giveBar(10,((-1*minW*this.width)/grWange)-5,(this.height*(maxH/grRange))-1,true));
			s += (MWJ_giveBar(this.height,((-1*minW*this.width)/grWange)-1,0,false)); }
		//end grid, start bars
		if( !this.type ) {
			for( var x = 0; x < this.data[0].length; x++ ) { for( var y = 0; y < this.data.length; y++ ) { if( this.data[y][x] ) {
				var avlWd = ( botTicks[x+1] - 1 ) - ( botTicks[x] + 1 ); var numExtraPixels = avlWd % this.data.length;
				...
				var offWidth = Math.floor( avlWd / this.data.length ) + ((y<numExtraPixels)?1:0);
				var offSide = botTicks[x] + 1 + ( y * Math.floor( avlWd / this.data.length ) ) + ((y<numExtraPixels)?y:numExtraPixels);
				s += (MWJ_giveBlock(this.rotate?offSet:offSide,this.rotate?offSide:offSet,this.rotate?scaleSize:offWidth,this.rotate?offWidth:scaleSize,this.colours[y],this.data[y][x]+(this.labels[y]?(' - '+this.labels[y]):'')+((this.XAxis&&this.XAxis[x])?(' - '+this.XAxis[x]):'')));
			} } }
		} else if( this.type == 1 || this.type == 2 ) {
			for( var x = 0; x < this.data[0].length; x++ ) { for( var y = (this.rotate?0:(this.data.length-1)), totData = 0; ( this.rotate && y < this.data.length ) || ( !this.rotate && y >= 0 ); y += (this.rotate?1:-1) ) { if( this.data[y][x] ) {
				var offWidth = ( botTicks[x+1] - 1 ) - ( botTicks[x] + 1 ); var offSide = botTicks[x] + 1; var totDiv = (this.type==1)?maxH:totCols[x];
				if( this.rotate ) { var offSet = Math.round((totData/totDiv)*this.width);
					...
					var scaleSize = Math.round(((totDiv-totData)/totDiv)*this.height) - offSet; }
				totData += this.data[y][x];
				s += (MWJ_giveBlock(this.rotate?offSet:offSide,this.rotate?offSide:offSet,this.rotate?scaleSize:offWidth,this.rotate?offWidth:scaleSize,this.colours[y],this.data[y][x]+' of '+totCols[x]+((this.type==2)?(' = '+(Math.round((this.data[y][x]/totCols[x])*10000)/100)+'%'):'')+(this.labels[y]?(' - '+this.labels[y]):'')+((this.XAxis&&this.XAxis[x])?(' - '+this.XAxis[x]):'')));
			} } }
		} else { for( var y = 0; y < this.data.length; y++ ) { for( var x = 0; x < this.data[y].length; x++ ) {
			var xPos = Math.round( ( ( this.data[y][x][0] - minW ) * this.width ) / grWange );
			var yPos = Math.round( ( ( maxH - this.data[y][x][1] ) * this.height ) / grRange );
			s += (MWJ_giveBlock(xPos-2,yPos-2,4,4,this.colours[y],'['+this.data[y][x][0]+','+this.data[y][x][1]+']'+(this.labels[y]?(' - '+this.labels[y]):'')));
		} } }
		//end bars
		s += ((document.layers&&!document.getElementById?'<\/ilayer>':'<\/div>')+'<\/td><\/tr><tr><td valign="top"'+(isIE5Mac?' height="20"':'')+'>'+(document.layers&&!document.getElementById?('<ilayer width="'+this.width+'" height="20">'):('<div style="position:relative;left:0px;top:0px;height:20px;width:'+this.width+'px;">')));
		//start bottom scale
		if( this.rotate ) {
			for( var x = 0; x < sideTicks.length; x++ ) { var y = (this.width*((sideTicks[x]-minH)/grRange))-((''+sideTicks[x]+((this.type==2)?'  ':'')).length*2);
				if( sideTicks[x] == minH ) { y += ((''+sideTicks[x]+'').length*2); } if( sideTicks[x] == maxH ) { y -= ((''+sideTicks[x]+((this.type==2)?'%':'')).length*4); } s += (MWJ_giveWords(y,5,sideTicks[x]+((this.type==2)?'%':''),true)); }
			var y = (this.width*(0-minH/grRange))-2;
			if( !minH ) { y += 2; } if( !maxH ) { y -= 4; } s += (MWJ_giveWords(y,5,0+((this.type==2)?'%':''),true));
		} else if( this.type == 3 ) {
			for( var x = 0; x < botTicks.length; x++ ) { var y = (this.width*((botTicks[x]-minW)/grWange))-((''+botTicks[x]+'').length*2);
				if( botTicks[x] == minW ) { y += ((''+botTicks[x]+'').length*2); } if( botTicks[x] == maxW ) { y -= ((''+botTicks[x]+'').length*4); } s += (MWJ_giveWords(y,5,botTicks[x],true)); }
		} else { for( var x = 0; this.XAxis && x < this.XAxis.length; x++ ) { if( this.XAxis[x] ) { s += (MWJ_giveWords( ( this.width * ( ( ( 2 * x ) + 1 ) / ( 2 * this.XAxis.length ) ) ) - ((''+this.XAxis[x]+'').length*3),5,this.XAxis[x],true)); } } }
		//end bottom scale - now end graph area
		s += ((document.layers&&!document.getElementById?'<\/ilayer>':'<\/div>')+'<\/td><\/tr><\/table><\/td>');

		//end of the graph area, resume chart decoration
		if( this.legend ) {
			s += ('<td'+((temp>1)?(' rowspan="'+temp+'"'):'')+'><table cellpadding="0" cellspacing="0" border="1"><tr><td>'+
				'<table cellpadding="0" cellspacing="4" border="0"><tr><th colspan="2">Legend<\/th><\/tr>');
			for( var x = 0, y = ''; x < this.data.length; x++ ) { y += '<tr><td bgcolor="'+this.colours[x]+'" width="20">&nbsp;<\/td><td>'+(this.labels[x]?this.labels[x]:'Untitled')+'<\/td><\/tr>'; }
			s += (y+'<\/table><\/td><\/tr><\/table>\n');
		}
		if( botTitle ) { s += ('<tr><th>'+botTitle+'<\/th><\/tr>'); }
		s += ('<\/table>\n');
		return(s);
	}
}
function MWJ_giveBar(oL,oX,oY,oR) {
	...
	if( document.layers&&!document.getElementById ) { return '<layer left="'+oX+'" top="'+oY+'" width="'+(oR?oL:1)+'" height="'+(oR?1:oL)+'" clip="0,0,'+(oR?oL:1)+','+(oR?1:oL)+'" bgcolor="#000000">&nbsp;<\/layer><layer left="'+(oX+(oR?0:1))+'" top="'+(oY+(oR?1:0))+'" width="'+(oR?oL:1)+'" height="'+(oR?1:oL)+'" clip="0,0,'+(oR?oL:1)+','+(oR?1:oL)+'" bgcolor="#ffffff">&nbsp;<\/layer>';
...
function MWJ_giveWords(oX,oY,oW,oE) {
	...
	if( document.layers&&!document.getElementById ) { return (oE?'':('<ilayer visibility="hide"><font size="-1">'+oW+'&nbsp; &nbsp;<\/font><\/ilayer><br>'))+'<layer left="'+oX+'" top="'+oY+'"><font size="-1">'+oW+'<\/font><\/layer>';
...
function MWJ_giveBlock(oX,oY,oW,oH,oC,oT) {
	if( document.layers&&!document.getElementById ) { return '<layer left="'+oX+'" top="'+oY+'" width="'+oW+'" height="'+oH+'" clip="0,0,'+oW+','+oH+'" bgcolor="'+oC+'" onmouseover="window.status=unescape(\''+escape(oT)+'\');" onmouseout="window.status=window.defaultStatus;">&nbsp;<\/layer>';
